...
public class AnimalInventoryManager : MonoBehaviour
{
    ...
    //동물 추가 함수
    public bool AddAnimal(AnimalData animalData, int amount)
    {
        ...
        AnimalInventorySlot existingSlot = animalList.Find(slot => slot.animalData == animalData);
        ...
        Debug.Log($"{animalData.animalName}이(가) 총 {amount}가 인벤토리에 추가되었습니다.");
        return true;
    }
    ...
    public void RemoveAnimal(AnimalData animalData, int amount)
    {
        ...
        AnimalInventorySlot targetSlot = animalList.Find(slot => slot.animalData == animalData);
        ...
        if (targetSlot != null)
        {
            ...
            {
                Debug.Log($"인벤토리에서 합성하려는 {animalData.animalName}의 갯수가 {amount - (targetSlot.currentCount - 1)}만큼 부족합니다.");
                return;
            }
        ...
        {
            Debug.LogWarning($"인벤토리에 합성하려는 {animalData.animalName}이(가) 존재하지 않습니다.");
        }
    }
}
