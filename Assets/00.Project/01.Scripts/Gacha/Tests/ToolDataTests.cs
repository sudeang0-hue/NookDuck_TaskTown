...
namespace TaskTown.Gacha.Tests
{
    ...
    {
        private static ToolData CreateTool(float baseCoinPerSecond, float levelBonusRatePerLevel, long levelUpBaseCost = 100, float levelUpCostIncreaseRate = 1.5f)
        {
        ...
        {
            ToolData tool = CreateTool(3f, 0.2f, levelUpBaseCost: 100, levelUpCostIncreaseRate: 1.5f);

            Assert.AreEqual(100, tool.CalculateLevelUpCoinCost(1));
        }
        ...
        public void CalculateLevelUpCoinCost_레벨이_오를수록_비용이_증가한다()
        {
            ...
            // 레벨 3 = 100 * 1.5^2 = 225
            Assert.AreEqual(225, tool.CalculateLevelUpCoinCost(3));
        }
    }
}
